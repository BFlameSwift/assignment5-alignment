...
def compute_per_instance_dpo_loss(
    ...
    prompt = [propmt_template.format(prompt=prompt)]
    response_chosen = [response_chosen+(tokenizer.eos_token or "")]
    response_rejected = [response_rejected+(tokenizer.eos_token or "")]
    ...
    chosen_ratio = lm_chosed_log_probs.sum() - lm_ref_chosed_log_probs.sum()
    rejected_ratio = lm_rejected_log_probs.sum() - lm_ref_rejected_log_probs.sum()
    # breakpoint()
    # input_pos = propmt_template.format(prompt=prompt, response=response_chosen)+(tokenizer.eos_token or "")
    # input_neg = propmt_template.format(prompt=prompt, response=response_rejected)+(tokenizer.eos_token or "")
